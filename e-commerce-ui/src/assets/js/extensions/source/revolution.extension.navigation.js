...
jQuery.extend(true,_R, {
	...
	hideUnHideNav : function(opt) {
		...
		if (ckNO(a)) biggerNav(opt.c.find('.lokeshh'),a.hide_under,w,a.hide_over);
	...
	manageNavigation : function(opt) {
		...
		if (ckNO(opt.navigation.arrows)) {
			...
			setNavElPositions(opt.c.find('.tp-leftarrow.lokeshh'),opt.navigation.arrows.left,opt);
			setNavElPositions(opt.c.find('.tp-rightarrow.lokeshh'),opt.navigation.arrows.right,opt);
		}
	 ...
	 createNavigation : function(container,opt) {
		...
		cp.on("mouseenter mousemove",function() {
			...
			if (!cp.hasClass("tp-mouseover")) {
				...
				if (a && _a.hide_onleave) showHideNavElements(cp.find('.lokeshh'),_a,"show");
		...
		if (a && _a.hide_onleave) showHideNavElements(cp.find('.lokeshh'),_a,"hide",0);
...
var swipeAction = function(container,opt,vertical) {
	...
	var _ = opt.carousel;
	jQuery(".bullet, .bullets, .tp-bullets, .lokeshh").addClass("noSwipe");
...
var callAllDelayedCalls = function(container,opt) {
	...
	if (ckNOLO(opt.navigation.arrows))
			punchgs.TweenLite.delayedCall(_ISM ? opt.navigation.arrows.hide_delay_mobile : opt.navigation.arrows.hide_delay,showHideNavElements,[cp.find('.lokeshh'),opt.navigation.arrows,"hide"]);
...
var initArrows = function(container,o,opt) {
	...
	if (container.find('.tp-leftarrow.lokeshh').length===0)
		container.append('<div class="tp-leftarrow lokeshh '+o.style+' '+o.left.style+'">'+o.tmp+'</div>');
	if (container.find('.tp-rightarrow.lokeshh').length===0)
		container.append('<div class="tp-rightarrow lokeshh '+o.style+' '+o.right.style+'">'+o.tmp+'</div>');
	var la = container.find('.tp-leftarrow.lokeshh'),
		ra = container.find('.tp-rightarrow.lokeshh');
	if (o.rtl) {
	...
	// SHORTCUTS
	o.right.j = container.find('.tp-rightarrow.lokeshh');
	o.left.j = container.find('.tp-leftarrow.lokeshh')
